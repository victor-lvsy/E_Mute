...
//
// Cache management : __CleanDCache() is used in many places here to invalidate cache manually
// so that we have two possibilities :
// 1) we don't use MPU or we use it except for the RAM (see mpu.c:74): in this case we must "#define __CleanDCache() SCB_CleanDCache()" and cache is indeed invalidated manually
// 2) or we use MPU including for the RAM, and manual cache invalidation is useless: in this case we must "#define __CleanDCache()" and cache is managed globally by the MPU
#ifndef FB_IN_SDRAM
	...
	//#define __CleanDCache() SCB_CleanDCache_by_Addr((uint32_t*)frameBuf0, LCD_SCREEN_WIDTH * LCD_SCREEN_HEIGHT*2) // TOO SLOW...
	#define __CleanDCache() SCB_CleanDCache() // we no longer need it if we're using the MPU to disable RAM cache
