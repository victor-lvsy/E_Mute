...
void MPU_Init(){
    ...
    MPU_InitStruct.SubRegionDisable = 0x00;
    MPU_InitStruct.DisableExec = MPU_INSTRUCTION_ACCESS_ENABLE;
    //HAL_MPU_ConfigRegion(&MPU_InitStruct); // commenting this line really speeds up audio calculation a lot but then we must invalidate cache manually for LCD with SCB_CleanDCache() ! (SR 27/11/2021)
